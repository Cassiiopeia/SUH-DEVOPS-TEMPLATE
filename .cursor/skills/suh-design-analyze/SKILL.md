---
name: suh-design-analyze
description: "Design Analyze Mode (Plan Only) - 시스템 설계 분석 전문가. 아키텍처, API, DB, UI/UX 설계를 분석하고 계획을 수립한다. 코드는 절대 작성하지 않는다. 설계 방향 결정이 필요할 때 사용. /suh-design-analyze 호출 시 사용. /design은 설계+구현을 함께 하는 별도 명령어."
---
...
산출물 md 저장 전:
```bash
PYTHONPATH="$PROJECT_ROOT/scripts" python3 -m suh_template.cli get-output-path design-analyze
```
