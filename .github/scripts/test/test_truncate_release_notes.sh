...
clen() { "$PYTHON" -c "import sys; print(len(open(sys.argv[1], encoding='utf-8').read()))" "$1"; }
cr() { tr -cd '\r' < "$1" | wc -c | tr -d ' '; }  # BSD wc는 앞에 공백 붙임 → 제거
ends_ellipsis() { "$PYTHON" -c "import sys; print('1' if open(sys.argv[1], encoding='utf-8').read().endswith(chr(8230)) else '0')" "$1"; }
...
"$PYTHON" -c "open('b.txt','w',encoding='utf-8').write('가'*600)"
...
"$PYTHON" -c "open('d.txt','w',encoding='utf-8').write('가나다라마'*20)"
bash "$SCRIPT" d.txt 100 byte
chk "바이트 <=100" "$(wc -c < d.txt)" le 100
if "$PYTHON" -c "open('d.txt',encoding='utf-8').read()" 2>/dev/null; then chk "UTF-8 디코드" 1 eq 1; else chk "UTF-8 디코드" 0 eq 1; fi
...
"$PYTHON" -c "open('hi.txt','w',encoding='utf-8').write('나'*600)"
