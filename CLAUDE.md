...
- 릴리스(배포)는 develop→main PR로만 진행한다 (`/projectops:changelog-deploy`). main 직접 push는 안전망(VERSION-CONTROL 가드)이 버전만 보전할 뿐 지원 경로가 아니다.
...
| 요청 유형 | 호출 스킬 |
|----------|----------|
| **PR 생성, PR 올려줘, 이슈 댓글, 댓글 달아줘, 이슈 확인, 이슈 닫기, PR 조회, GitHub API** | **`projectops:github` ← 최우선 트리거** |
| 코드 분석, 현황 파악 | `projectops:analyze` |
| 버그, 오류, 원인 파악 | `projectops:troubleshoot` |
| 새 기능 설계 | `projectops:plan` → `projectops:implement` |
| 코드 리뷰 | `projectops:review` |
| 이슈 작성 | `projectops:issue` |
| 커밋 | `projectops:commit` |
| 배포 / automerge 실패 재트리거 | `projectops:changelog-deploy` |
| 보고서 | `projectops:report` |
| 원격 서버 SSH 접속, 로그/상태 확인 | `projectops:ssh` |
...
### 1. `projectops:github` 스킬 자동 트리거 실패

**문제**: 사용자가 "PR 올려줘", "댓글 달아줘" 등을 요청해도 `projectops:github` 스킬이 자동으로 트리거되지 않고, 다른 스킬(brainstorming 등)이 먼저 실행됨.

**원인**: `superpowers:using-superpowers` 규칙에서 어떤 스킬이든 1% 가능성이면 먼저 호출하도록 강제되는데, `brainstorming` 등 범용 스킬이 더 넓은 설명을 가지고 있어 우선 매칭됨. `projectops:github` description의 트리거 키워드("PR 만들어줘", "댓글 달아줘")가 있어도 다른 스킬보다 낮은 우선순위로 처리됨.

**해결 방법 (사용자 관점)**:
- GitHub 작업 시 명시적으로 `/projectops:github` 슬래시 커맨드를 입력
- 또는 메시지 앞에 "github:" 접두어 사용 ("github: PR 올려줘")

**해결 방법 (스킬 개발 관점)**:
- `skills/github/SKILL.md`의 description에 더 구체적인 트리거 키워드 추가 필요
- 또는 `projectops:github`를 Skill routing 표에 더 명확한 패턴으로 등록
- PR 생성, 이슈 댓글, GitHub API 작업은 **반드시 `/projectops:github` 명시 호출** 원칙을 CLAUDE.md에 명시

### 2. `projectops:github` 스킬 실행 시 Repo 자동 감지 실패 (워크트리 환경)
...
**해결 방법**:
- 스킬 호출 시 대상 레포를 명시: `/projectops:github TEAM-ROMROM/RomRom-BE 이슈 #653 PR 생성`
