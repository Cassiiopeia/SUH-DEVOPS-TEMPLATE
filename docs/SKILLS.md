...
예를 들어 `/cassiiopeia:suh-review`를 실행하면 Claude가 단순히 "코드 좀 봐줘"에 답하는 게 아니라, **보안/성능/버그/품질/아키텍처/테스트 6가지 관점**으로 나눠서 **Critical/Major/Minor 우선순위**로 정리된 리뷰를 돌려줍니다.
...
- `/cassiiopeia:suh-design`의 구현 없이 분석만 하는 버전
...
- 실제 작업은 `/cassiiopeia:suh-refactor`로 진행
...
**무엇을 하나요?**
`/cassiiopeia:suh-plan`이나 `/cassiiopeia:suh-analyze`에서 나온 계획을 **실제 코드로 구현**합니다. 기존 프로젝트의 코드 스타일(들여쓰기, 네이밍, 패턴)을 자동으로 감지해서 100% 맞춰 작성합니다.
...
- 분석만 원한다면 `/cassiiopeia:suh-design-analyze`를 쓰세요
...
- 계획만 보고 싶다면 `/cassiiopeia:suh-refactor-analyze`
...
**무엇을 하나요?**
`/cassiiopeia:suh-issue`나 `/cassiiopeia:suh-init-worktree`가 저장해둔 이슈 컨텍스트(`.suh-template/context/current-issue.json`)를 읽어서 **프로젝트 커밋 컨벤션에 맞는 메시지를 자동 완성**하고 커밋합니다. 메시지는 반드시 제안 후 사용자 확인을 받고 실행합니다.
...
**이슈 컨텍스트가 없을 때**:
1. 이슈 새로 만들기 (`/cassiiopeia:suh-issue` 플로우 안내)
