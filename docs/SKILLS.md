...
> GitHub Actions 자동화 템플릿과 더불어, Claude Code에서 바로 사용할 수 있는 개발/DevOps Skill 20종을 제공합니다.
...
이슈 등록부터 커밋, 보고서까지 — 반복 작업을 AI가 대신합니다.
...
| 상황 | 추천 흐름 |
|------|----------|
| **새 기능 개발 (표준)** | `issue` → `init-worktree` → `plan` → `implement` → `test` → `review` → `commit` → `report` |
| **버그 수정** | `issue` → `init-worktree` → `troubleshoot` → `implement` → `test` → `commit` → `report` |
| **리팩토링** | `issue` → `init-worktree` → `refactor-analyze` → `refactor` → `test` → `review` → `commit` → `report` |
| **설계부터 시작** | `issue` → `init-worktree` → `design-analyze` → `design` → `test` → `review` → `commit` → `report` |
