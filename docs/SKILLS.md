...
**무엇을 하나요?**
`/projectops:issue`나 `/projectops:init-worktree`가 저장해둔 이슈 컨텍스트(`.projectops/context/current-issue.json`)를 읽어서 **프로젝트 커밋 컨벤션에 맞는 메시지를 자동 완성**하고 커밋합니다. 메시지는 반드시 제안 후 사용자 확인을 받고 실행합니다.
