...
| SKILL | 패턴 | 상태 |
|---|---|---|
| troubleshoot | `PYTHONPATH="$SCRIPTS_PATH" $PYTHON -m ...` | ❌ **`$SCRIPTS_PATH`·`$PYTHON` 미정의 사용**. 실측 결과: `Exit 127: -m: command not found` |
| review | 동일 | ❌ 동일 깨짐 |
| issue | `cd "$PROJECT_ROOT/scripts" && ...` | ⚠️ Phase 0(`PROJECT_ROOT` 정의) 건너뛰면 `cd /scripts` 실패 → `ModuleNotFoundError` |
| github | 동일 | ⚠️ 7개 코드블록 모두 동일 위험 (가장 위험) |
| commit | 동일 | ⚠️ 1단계→3단계 순차 의존 |
| report | 동일 | ⚠️ 동일 |
| changelog-deploy | **매 블록 self-contained** (`PROJECT_ROOT`·`PYTHON` 재선언) | ✅ 유일하게 강건. 실측 통과 |
...
| 파일 | 변경 |
|---|---|
| `skills/troubleshoot/SKILL.md` L99-102 | `$SCRIPTS_PATH`·`$PYTHON` 미정의 블록 → self-contained 5줄로 교체 |
| `skills/review/SKILL.md` L88-91 | 동일 교체 |
| `skills/issue/SKILL.md` | 모든 코드블록(3개) 앞에 PROJECT_ROOT+PYTHON 재선언 + `[ -z "$PYTHON" ]` 가드 |
| `skills/github/SKILL.md` | 모든 코드블록(7개) self-contained화 (가장 위험 → 우선순위 1) |
| `skills/commit/SKILL.md` | 3단계 이슈 조회 블록 self-contained화 |
| `skills/report/SKILL.md` | 포스팅 플로우 블록 self-contained화 |
| `skills/changelog-deploy/SKILL.md` | **touch 안 함** (이미 self-contained — 참조 reference 역할) |
...
- `skills/ssh/scripts/ssh_connect.py` — 별개 패턴 (이미 절대경로 풀이로 OK)
...
2. `skills/troubleshoot/SKILL.md` 깨진 호출 패턴 교체
3. `skills/review/SKILL.md` 깨진 호출 패턴 교체
4. `skills/issue/SKILL.md` 코드블록 self-contained화
5. `skills/github/SKILL.md` 7개 코드블록 self-contained화 (우선순위 1)
6. `skills/commit/SKILL.md` 코드블록 self-contained화
7. `skills/report/SKILL.md` 코드블록 self-contained화
