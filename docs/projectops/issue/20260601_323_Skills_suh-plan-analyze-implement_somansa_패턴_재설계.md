...
기존 `suh-plan` / `suh-analyze` / `suh-implement` 3종 스킬은 가볍지만 다음 한계가 있다.

- **책임 분리 모호**: `suh-plan`(전략)과 `suh-analyze`(분석 + 구현 계획)의 책임이 겹친다. 구현 계획서를 plan에도, analyze에도 쓸 수 있어 호출자가 매번 헷갈린다.
...
- **plan/analyze 자동 로드 없음**: `suh-implement`가 `docs/suh-template/` 폴더를 자동 스캔하지 않아 매번 사용자가 산출물 파일을 명시적으로 첨부해야 한다.
- **브랜치 가드 없음**: 보호 브랜치(`main`/`master`/`develop`/`*release*`) 위에서 곧장 implement가 시작되는 사고가 가능하다.
- **Finishing 단계 없음**: 구현 완료 후 PR 생성·worktree 정리가 수동이며, `suh-github`·`suh-init-worktree` 같은 기존 스킬과의 매끄러운 연동 흐름이 없다.
...
`suh-plan` / `suh-analyze` / `suh-implement` 3종 스킬을 somansa 패턴으로 풀 포팅하되, 회사 전용 요소는 SUH 일반 GitHub 환경으로 매핑한다.
...
- `suh-implement` = DO — 코드 자체가 결과 (별도 산출 md 없음)

**주요 도입 패턴**

- **HARD-GATE**: suh-plan에 HOW 침범 차단, suh-analyze에 No-Placeholder 차단, suh-implement에 설계 필수 게이트
...
- **Finishing Phase**: implement Phase 6 — 최종 테스트 / 환경 감지 / 4옵션(PR / 로컬 머지 / 보관 / 폐기) / worktree 정리 / `/suh-report` 안내
