...
- `skills/suh-issue/scripts/issue_cli.py`의 `get-next-seq` 서브커맨드 호출 시 agent가 `<skill_id>` 1개 인자 대신 `<PROJECT_ROOT> <YYYYMMDD>` 2개 인자를 넘겨 `Error: Exit code 2 / unrecognized arguments` 발생. 실제 발생 케이스 — 2026-06-02 changelog-deploy 승인 게이트 이슈 작성 도중 재현.
...
- `get-next-seq`는 `suh-issue` SKILL.md 어디에서도 호출 예시가 명시돼 있지 않다. SKILL.md 4단계는 "임시 번호 `TMP1`, `TMP2` 사용 후 GitHub 등록 후 rename"으로 충돌되는 절차가 적혀 있다. 그럼에도 서브커맨드가 CLI에 노출돼 있어 agent가 "있으니 써야 한다"고 잘못 추론한다.
- 같은 함정이 다른 서브커맨드에도 잠재해 있다. `issue_cli.py`의 `normalize-title`, `create-branch-name`, `get-commit-template`는 `suh-issue/SKILL.md`에 호출 예시가 0건. agent가 시그니처를 추측해 깨질 가능성이 동일하게 남아 있다.
...
3. SKILL.md ↔ CLI 서브커맨드 노출 규칙을 표준화한다.
   ...
   - 점검 대상 우선순위: `suh-issue`(`normalize-title`, `create-branch-name`, `get-commit-template` 호출예 없음), 이후 다른 skill 전수.
