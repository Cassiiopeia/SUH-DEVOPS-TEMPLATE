...
기존 `changelog-deploy`는 5단계에서 릴리스 노트 파일을 작성한 직후 6단계 PR 생성으로 즉시 진행해 사용자가 본문을 검토할 시점이 없었다. 자동 모드를 원하는 사용자도 있어 일률 강제가 부적절. **승인 게이트(5.5단계 / fix 4.5단계) + 자동/수동 토글 + 첫 실행 자동화 제안**을 도입했다.
...
- `skills/changelog-deploy/SKILL.md` — 5.5단계 / fix 4.5단계 신규, 핵심 원칙 갱신, 시작 전 §3 자동 승인 판정 추가
