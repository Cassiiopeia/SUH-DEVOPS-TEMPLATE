...
flowchart TD
    ...
    B --> C["플러그인 캐시 탐색<br/>~/.claude/plugins/cache/*/projectops/*/skills/SKILL/scripts"]
    C --> D{"캐시에 있음?"}
    D -->|예| E["캐시 최신버전 경로 사용"]
...
### 스킬별 SKILL.md
- `skills/github/SKILL.md`: 코드블록 10곳 교체 + "시작 전" 섹션에 캐시 설치 위치 안내 추가 + Windows 주의 주석의 틀린 고정 경로 수정.
- `skills/issue/SKILL.md`: 코드블록 3곳 교체 + Windows 주의 주석 2곳 수정.
- `skills/commit/SKILL.md`: 코드블록 교체.
- `skills/report/SKILL.md`: 코드블록 교체.
- `skills/review/SKILL.md`: 코드블록 교체.
- `skills/troubleshoot/SKILL.md`: 코드블록 교체.
- `skills/changelog-deploy/SKILL.md`: 코드블록 5곳 교체.
...
수정한 7개 스킬 CLI를 "skills 폴더가 없는 다른 레포"로 PROJECT_ROOT를 지정한 환경에서 실제 호출해, 캐시 탐색 → import → CLI 실행이 모두 성공함을 확인했다 (github / issue / commit / report / review / troubleshoot / changelog-deploy).
