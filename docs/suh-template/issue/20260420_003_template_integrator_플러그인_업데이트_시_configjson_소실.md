...
`template_integrator.ps1`에서 플러그인 업데이트(`claude plugin update`) 또는 Claude Code UI의 "Update Now" 버튼으로 업데이트 시, 새 버전 캐시 폴더에 기존 `config.json`이 복사되지 않아 API 키 등 설정값이 초기화되는 문제.
...
유사 케이스로 `somansa-claude-code` 레포에서 동일 문제를 `install.ps1` + 런타임 마이그레이션(`config_migrate.py`)으로 해결한 사례 있음.

🔄 재현 방법
---

1. cassiiopeia 플러그인 설치 후 특정 skill의 `config.json`에 API 키 등 설정 저장
2. `template_integrator.ps1` 실행 후 "업데이트" 선택 또는 Claude Code UI에서 "Update Now" 클릭
3. 업데이트 완료 후 해당 skill 실행 시 config not found 오류 발생

📸 참고 자료
---

- 유사 구현 레포: `somansa-claude-code`
  - `install.ps1`: `Invoke-ConfigMigration` 함수 — 설치 완료 후 이전 버전 캐시에서 config.json 복사
  - `common/config_migrate.py`: 런타임 마이그레이션 — skill 실행 시 config 없으면 이전 버전에서 탐색·복사
  - 각 skill `load_config()` 시점에 `migrate_config_if_needed()` 호출

✅ 예상 동작
---

- 플러그인 업데이트 후에도 기존 config.json 설정값(API 토큰 등)이 새 버전 폴더로 자동 이관됨
