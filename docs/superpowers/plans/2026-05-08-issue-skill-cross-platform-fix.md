...
**Issue:** [#288](https://github.com/Cassiiopeia/SUH-DEVOPS-TEMPLATE/issues/288)
...
keyword = "$KEYWORD"
encoded = urllib.parse.quote(keyword, safe='')
url = f"https://api.github.com/search/issues?q=is:issue+repo:Cassiiopeia/SUH-DEVOPS-TEMPLATE+in:title+{encoded}&per_page=2"
req = urllib.request.Request(url)
...
```
커밋 메시지:
issue 스킬 중복검색 단계 Windows python3 호환 누락 : fix : 2-1·4-1단계 PYTHON 검출 + urllib heredoc 패턴으로 통일, ssh L104 동반 점검, v3.0.37 bump https://github.com/Cassiiopeia/SUH-DEVOPS-TEMPLATE/issues/288
...
```bash
git commit -m "issue 스킬 중복검색 단계 Windows python3 호환 누락 : fix : 2-1·4-1단계 PYTHON 검출 + urllib heredoc 패턴으로 통일, ssh L104 동반 점검, v3.0.37 bump https://github.com/Cassiiopeia/SUH-DEVOPS-TEMPLATE/issues/288"
```
...
pat = "GHP_PLACEHOLDER"
url = "https://api.github.com/repos/Cassiiopeia/SUH-DEVOPS-TEMPLATE/issues/288/comments"
body = """v3.0.37 패치 완료.
