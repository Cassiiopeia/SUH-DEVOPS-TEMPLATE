...
**Goal:** `changelog-deploy` 스킬이 매 배포마다 `/tmp`에 즉석 생성하던 PR 상태·automerge·deploy 반영 확인 스크립트를, 재사용 가능한 `deploy-status` 서브커맨드 하나로 통합한다.
...
- **Modify** `skills/changelog-deploy/SKILL.md` — deploy 모드 7단계(검증) 삽입, fix 모드 1단계 교체, 주의사항 추가.
...
Run: `grep -n "deploy-status\|### .*단계" skills/changelog-deploy/SKILL.md`
