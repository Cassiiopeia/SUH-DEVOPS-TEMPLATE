...
| **Phase 6** | skill-creator templates 업데이트 | Phase 5 | 2 |
...
root = find_scripts_root(Path('skills/github'))
print(root)
...
**Files:**
- Create: `skills/github/scripts/github_cli.py`
- Create: `skills/github/tests/test_github_cli.py`

- [ ] **Step 1: 폴더 생성**

```bash
PROJECT_ROOT=$(git rev-parse --show-toplevel)
mkdir -p "$PROJECT_ROOT/skills/github/scripts"
mkdir -p "$PROJECT_ROOT/skills/github/tests"
```
...
_HERE = Path(__file__).resolve()
_PROJECT_ROOT = _HERE.parents[3]  # skills/github/scripts/github_cli.py → 3 up
...
```bash
cd "$PROJECT_ROOT/skills/github/scripts" && python github_cli.py --help 2>&1 | head -10
```
...
```bash
cd "$PROJECT_ROOT" && python skills/github/scripts/github_cli.py --help 2>&1 | head -5
```
...
```bash
git add skills/github/scripts/github_cli.py
git commit -m "feat(github): github_cli 골격 + bootstrap https://github.com/Cassiiopeia/projectops/issues/322"
```
...
**Files:**
- Modify: `skills/github/scripts/github_cli.py` (서브커맨드 추가)
- Create: `skills/github/tests/test_github_cli.py`

- [ ] **Step 1: Write failing test**

```python
# skills/github/tests/test_github_cli.py
"""github_cli 단위 테스트.
...
```bash
cd "$PROJECT_ROOT" && python -m pytest skills/github/tests/test_github_cli.py -v 2>&1 | tail -15
```
...
```bash
cd "$PROJECT_ROOT" && python -m pytest skills/github/tests/test_github_cli.py -v 2>&1 | tail -10
```
...
```bash
cd "$PROJECT_ROOT/skills/github/scripts" && python github_cli.py get-issue Cassiiopeia SUH-DEVOPS-TEMPLATE 322 2>&1 | python -c "import sys, json; d = json.loads(sys.stdin.read()); print('ok:', d['ok'], '/ title:', d.get('title', '')[:50])"
```
...
```bash
git add skills/github/scripts/github_cli.py skills/github/tests/test_github_cli.py
git commit -m "feat(github): get-issue 서브커맨드 + tests https://github.com/Cassiiopeia/projectops/issues/322"
```
...
- Modify: `skills/github/tests/test_github_cli.py`
...
```bash
cd "$PROJECT_ROOT/skills/github/scripts" && python github_cli.py get-issues Cassiiopeia SUH-DEVOPS-TEMPLATE 322 321 320 2>&1 | python -c "import sys, json; d = json.loads(sys.stdin.read()); print('count:', d.get('count'))"
```
...
```bash
git add skills/github/scripts/github_cli.py
git commit -m "feat(github): get-issues 서브커맨드 https://github.com/Cassiiopeia/projectops/issues/322"
```
...
```bash
cd "$PROJECT_ROOT/skills/github/scripts" && python github_cli.py update-issue Cassiiopeia SUH-DEVOPS-TEMPLATE 322 --assignees Cassiiopeia 2>&1 | python -c "import sys, json; d = json.loads(sys.stdin.read()); print('ok:', d['ok'])"
```
...
```bash
git add skills/github/scripts/github_cli.py
git commit -m "feat(github): update-issue 서브커맨드 https://github.com/Cassiiopeia/projectops/issues/322"
```
...
cd "$PROJECT_ROOT/skills/github/scripts" && python github_cli.py add-comment Cassiiopeia SUH-DEVOPS-TEMPLATE 322 /tmp/test_comment.md 2>&1 | python -c "import sys, json; d = json.loads(sys.stdin.read()); print('ok:', d['ok'], '/ url:', d.get('url', ''))"
```
...
```bash
git add skills/github/scripts/github_cli.py
git commit -m "feat(github): add-comment + create-pr 서브커맨드 https://github.com/Cassiiopeia/projectops/issues/322"
```
...
```bash
cd "$PROJECT_ROOT/skills/github/scripts"
...
```bash
git add skills/github/scripts/github_cli.py
git commit -m "feat(github): list-prs + update-pr + search-issues 서브커맨드 https://github.com/Cassiiopeia/projectops/issues/322"
```
...
```bash
cd "$PROJECT_ROOT/skills/github/scripts" && python github_cli.py explore repo-detail Cassiiopeia SUH-DEVOPS-TEMPLATE 2>&1 | python -c "import sys, json; d = json.loads(sys.stdin.read()); print('ok:', d['ok'], '/ summary:', d.get('summary'))"
```
...
```bash
git add skills/github/scripts/github_cli.py
git commit -m "feat(github): explore 서브커맨드 (5종 중첩) https://github.com/Cassiiopeia/projectops/issues/322"
```
...
```bash
cd "$PROJECT_ROOT/skills/github/scripts" && python github_cli.py secrets list Cassiiopeia SUH-DEVOPS-TEMPLATE 2>&1 | python -c "import sys, json; d = json.loads(sys.stdin.read()); print('ok:', d['ok'], '/ count:', d.get('count'))"
```
...
```bash
git add skills/github/scripts/github_cli.py
git commit -m "feat(github): secrets 서브커맨드 (list/set) https://github.com/Cassiiopeia/projectops/issues/322"
```
...
```powershell
wsl bash -c "cd /mnt/d/0-suh/project/suh-github-template/skills/github/scripts && python3 github_cli.py --help | head -5"
```
...
```powershell
wsl bash -c "cd /mnt/d/0-suh/project/suh-github-template/skills/github/scripts && python3 github_cli.py search-issues Cassiiopeia SUH-DEVOPS-TEMPLATE 'skill py' 2>&1 | tail -1" | python -c "import sys, json; d = json.loads(sys.stdin.read()); print('WSL ok:', d['ok'])"
```
...
**Files:**
- Create: `skills/issue/scripts/issue_cli.py`

- [ ] **Step 1: github_cli.py를 issue_cli.py로 복사 후 서브커맨드 교체**

```bash
PROJECT_ROOT=$(git rev-parse --show-toplevel)
mkdir -p "$PROJECT_ROOT/skills/issue/scripts"
cp "$PROJECT_ROOT/skills/github/scripts/github_cli.py" "$PROJECT_ROOT/skills/issue/scripts/issue_cli.py"
```
...
```python
parser = argparse.ArgumentParser(prog="issue_cli", description="issue skill CLI")
```

- [ ] **Step 2: smoke test 6개 서브커맨드**

```bash
cd "$PROJECT_ROOT/skills/issue/scripts"
...
```bash
git add skills/issue/scripts/issue_cli.py
git commit -m "feat(issue): issue_cli.py 6개 서브커맨드 https://github.com/Cassiiopeia/projectops/issues/322"
```
...
**Files:**
- Create: `skills/commit/scripts/commit_cli.py`

- [ ] **Step 1: 골격 복사 + 서브커맨드 교체**

```bash
PROJECT_ROOT=$(git rev-parse --show-toplevel)
mkdir -p "$PROJECT_ROOT/skills/commit/scripts"
cp "$PROJECT_ROOT/skills/github/scripts/github_cli.py" "$PROJECT_ROOT/skills/commit/scripts/commit_cli.py"
```
...
prog 교체:
```python
parser = argparse.ArgumentParser(prog="commit_cli", description="commit skill CLI")
```

- [ ] **Step 2: smoke test**

```bash
cd "$PROJECT_ROOT/skills/commit/scripts"
...
```bash
git add skills/commit/scripts/commit_cli.py
git commit -m "feat(commit): commit_cli.py 4개 서브커맨드 https://github.com/Cassiiopeia/projectops/issues/322"
```
...
**Files:**
- Create: `skills/report/scripts/report_cli.py`

- [ ] **Step 1: 골격 복사 + 서브커맨드 교체**

```bash
PROJECT_ROOT=$(git rev-parse --show-toplevel)
mkdir -p "$PROJECT_ROOT/skills/report/scripts"
cp "$PROJECT_ROOT/skills/github/scripts/github_cli.py" "$PROJECT_ROOT/skills/report/scripts/report_cli.py"
```
...
prog 교체:
```python
parser = argparse.ArgumentParser(prog="report_cli", description="report skill CLI")
```

- [ ] **Step 2: smoke test**

```bash
cd "$PROJECT_ROOT/skills/report/scripts" && python report_cli.py get-output-path report 2>&1 | python -c "import sys, json; print(json.loads(sys.stdin.read())['path'])"
```
...
```bash
git add skills/report/scripts/report_cli.py
git commit -m "feat(report): report_cli.py 2개 서브커맨드 https://github.com/Cassiiopeia/projectops/issues/322"
```
...
**Files:**
- Create: `skills/review/scripts/review_cli.py`

- [ ] **Step 1: report_cli 복사 후 skill_id 기본값만 review로**

```bash
PROJECT_ROOT=$(git rev-parse --show-toplevel)
mkdir -p "$PROJECT_ROOT/skills/review/scripts"
cp "$PROJECT_ROOT/skills/report/scripts/report_cli.py" "$PROJECT_ROOT/skills/review/scripts/review_cli.py"
```
...
```python
parser = argparse.ArgumentParser(prog="review_cli", description="review skill CLI")
```

- [ ] **Step 2: smoke test**

```bash
cd "$PROJECT_ROOT/skills/review/scripts" && python review_cli.py get-output-path review 2>&1 | python -c "import sys, json; print(json.loads(sys.stdin.read())['path'])"
```
...
```bash
git add skills/review/scripts/review_cli.py
git commit -m "feat(review): review_cli.py get-output-path https://github.com/Cassiiopeia/projectops/issues/322"
```
...
**Files:**
- Create: `skills/troubleshoot/scripts/troubleshoot_cli.py`

- [ ] **Step 1: review_cli 복사 후 skill_id 기본값만 troubleshoot로**

```bash
PROJECT_ROOT=$(git rev-parse --show-toplevel)
mkdir -p "$PROJECT_ROOT/skills/troubleshoot/scripts"
cp "$PROJECT_ROOT/skills/review/scripts/review_cli.py" "$PROJECT_ROOT/skills/troubleshoot/scripts/troubleshoot_cli.py"
```
...
prog 교체:
```python
parser = argparse.ArgumentParser(prog="troubleshoot_cli", description="troubleshoot skill CLI")
```

- [ ] **Step 2: smoke test**

```bash
cd "$PROJECT_ROOT/skills/troubleshoot/scripts" && python troubleshoot_cli.py get-output-path troubleshoot 2>&1 | python -c "import sys, json; print(json.loads(sys.stdin.read())['path'])"
```
...
```bash
git add skills/troubleshoot/scripts/troubleshoot_cli.py
git commit -m "feat(troubleshoot): troubleshoot_cli.py get-output-path https://github.com/Cassiiopeia/projectops/issues/322"
```
...
**Files:**
- Create: `skills/changelog-deploy/scripts/changelog_cli.py`

- [ ] **Step 1: 골격 복사**

```bash
PROJECT_ROOT=$(git rev-parse --show-toplevel)
mkdir -p "$PROJECT_ROOT/skills/changelog-deploy/scripts"
cp "$PROJECT_ROOT/skills/github/scripts/github_cli.py" "$PROJECT_ROOT/skills/changelog-deploy/scripts/changelog_cli.py"
```
...
```python
parser = argparse.ArgumentParser(prog="changelog_cli", description="changelog-deploy skill CLI")
```

- [ ] **Step 2: smoke test**

```bash
cd "$PROJECT_ROOT/skills/changelog-deploy/scripts"
...
```bash
git add skills/changelog-deploy/scripts/changelog_cli.py
git commit -m "feat(changelog-deploy): changelog_cli.py 5개 서브커맨드 https://github.com/Cassiiopeia/projectops/issues/322"
```
...
### Task 4.1: github SKILL.md 재작성

**Files:**
- Modify: `skills/github/SKILL.md`

- [ ] **Step 1: 모든 코드블록 찾기**

```bash
grep -n "suh_template.suh_command" skills/github/SKILL.md
```
...
```bash
grep -c "github_cli.py" skills/github/SKILL.md
grep -c "suh_template.suh_command" skills/github/SKILL.md
```
...
git commit -m "docs(github): SKILL.md 표준 5줄 호출 패턴 적용 https://github.com/Cassiiopeia/projectops/issues/322"
```

---

### Task 4.2: issue SKILL.md 재작성
...
```bash
grep -n "suh_template.suh_command" skills/issue/SKILL.md
```

- [ ] **Step 2: 표준 5줄 패턴으로 교체**

각 호출의 cwd = `$PROJECT_ROOT/skills/issue/scripts`, 명령 = `python issue_cli.py <subcmd>`.

기존 `create-issue`, `search-issues`, `update-issue` 호출 → issue_cli.py로 매핑.

단 `update-issue` (담당자 지정용) 은 issue에서 호출하므로 issue_cli.py에도 추가 필요 — 또는 github_cli.py로 위임 가능. 이 경우 SKILL.md 일부 블록은 cwd가 `github/scripts`가 될 수 있음. **간결성 위해 issue_cli.py에 update-issue 추가 권장.** Task 3.1 시점에 빠진 거 보강:
...
```bash
git add skills/issue/SKILL.md skills/issue/scripts/issue_cli.py
git commit -m "docs(issue): SKILL.md 표준 5줄 패턴 + update-issue 추가 https://github.com/Cassiiopeia/projectops/issues/322"
```

---

### Task 4.3: commit SKILL.md 재작성
...
```bash
grep -n "suh_template.suh_command" skills/commit/SKILL.md
```

- [ ] **Step 2: 표준 5줄 패턴 + cwd = commit/scripts**
...
git commit -m "docs(commit): SKILL.md 표준 5줄 패턴 적용 https://github.com/Cassiiopeia/projectops/issues/322"
```

---

### Task 4.4: report SKILL.md 재작성
...
cwd = `$PROJECT_ROOT/skills/report/scripts`, 명령 = `python report_cli.py add-comment ...` / `python report_cli.py get-output-path ...`

- [ ] **Step 2: 검증**

```bash
cd "$(git rev-parse --show-toplevel)/skills/report/scripts" && python report_cli.py get-output-path report 2>&1 | python -c "import sys, json; print(json.loads(sys.stdin.read())['path'])"
```
...
git commit -m "docs(report): SKILL.md 표준 5줄 패턴 적용 https://github.com/Cassiiopeia/projectops/issues/322"
```

---

### Task 4.5: review SKILL.md 재작성
...
PYTHON=$(for _py in python3 python; do _path=$(command -v "$_py" 2>/dev/null) || continue; "$_path" -c "import sys; sys.exit(0)" 2>/dev/null && echo "$_path" && break; done)
[ -z "$PYTHON" ] && { echo "Python not found"; exit 1; }
cd "$PROJECT_ROOT/skills/review/scripts" || exit 1
...
```bash
cd "$(git rev-parse --show-toplevel)/skills/review/scripts" && python review_cli.py get-output-path review 2>&1 | python -c "import sys, json; print(json.loads(sys.stdin.read())['path'])"
```
...
git commit -m "docs(review): SKILL.md 깨진 호출 정정 (변수 미정의 → 표준 5줄) https://github.com/Cassiiopeia/projectops/issues/322"
```

---

### Task 4.6: troubleshoot SKILL.md 재작성
...
PYTHON=$(for _py in python3 python; do _path=$(command -v "$_py" 2>/dev/null) || continue; "$_path" -c "import sys; sys.exit(0)" 2>/dev/null && echo "$_path" && break; done)
[ -z "$PYTHON" ] && { echo "Python not found"; exit 1; }
cd "$PROJECT_ROOT/skills/troubleshoot/scripts" || exit 1
...
```bash
cd "$(git rev-parse --show-toplevel)/skills/troubleshoot/scripts" && python troubleshoot_cli.py get-output-path troubleshoot 2>&1 | python -c "import sys, json; print(json.loads(sys.stdin.read())['path'])"
```
...
```bash
git add skills/troubleshoot/SKILL.md
git commit -m "docs(troubleshoot): SKILL.md 깨진 호출 정정 https://github.com/Cassiiopeia/projectops/issues/322"
```

---

### Task 4.7: changelog-deploy SKILL.md 재작성

**Files:**
- Modify: `skills/changelog-deploy/SKILL.md`

- [ ] **Step 1: 코드블록 위치**

```bash
grep -n "suh_template.suh_command" skills/changelog-deploy/SKILL.md
```
...
cwd = `$PROJECT_ROOT/skills/changelog-deploy/scripts`, 명령 = `python changelog_cli.py <subcmd>`

- [ ] **Step 3: 검증**

```bash
cd "$(git rev-parse --show-toplevel)/skills/changelog-deploy/scripts" && python changelog_cli.py deploy-status Cassiiopeia SUH-DEVOPS-TEMPLATE 2>&1 | python -c "import sys, json; print('verdict:', json.loads(sys.stdin.read()).get('verdict'))"
```
...
```bash
git add skills/changelog-deploy/SKILL.md
git commit -m "docs(changelog-deploy): SKILL.md 표준 5줄 패턴 + changelog_cli https://github.com/Cassiiopeia/projectops/issues/322"
```
...
```powershell
wsl bash -c "cd /mnt/d/0-suh/project/suh-github-template/skills/troubleshoot/scripts && python3 troubleshoot_cli.py get-output-path troubleshoot 2>&1 | tail -1"
```
...
## Phase 6: skill-creator templates 업데이트

### Task 6.1: python_cli_script.py 새 표준 골격으로 교체

**Files:**
- Modify: `skills/skill-creator/templates/python_cli_script.py`
...
cp "$PROJECT_ROOT/skills/skill-creator/templates/python_cli_script.py" /tmp/test_skill/scripts/test_cli.py
# bootstrap은 _HERE.parents[3] 의존이므로 임시 skill을 실제 skills/ 아래에 둬야 함
mkdir -p "$PROJECT_ROOT/skills/_test_skill_template/scripts"
cp "$PROJECT_ROOT/skills/skill-creator/templates/python_cli_script.py" "$PROJECT_ROOT/skills/_test_skill_template/scripts/test_cli.py"
...
```bash
git add skills/skill-creator/templates/python_cli_script.py
git commit -m "feat(skill-creator): templates/python_cli_script.py 새 표준 골격 https://github.com/Cassiiopeia/projectops/issues/322"
```

---

### Task 6.2: skill-creator SKILL.md 새 표준 반영
...
```bash
grep -n "PYTHONPATH\|suh_template.suh_command" "$PROJECT_ROOT/skills/skill-creator/SKILL.md"
```
...
git commit -m "docs(skill-creator): SKILL.md 새 표준 안내 추가 https://github.com/Cassiiopeia/projectops/issues/322"
```
...
```bash
PROJECT_ROOT=$(git rev-parse --show-toplevel)
cd "$PROJECT_ROOT/skills/troubleshoot/scripts" && python troubleshoot_cli.py get-output-path troubleshoot 2>&1 | tail -1
cd "$PROJECT_ROOT/skills/github/scripts" && python github_cli.py get-issue Cassiiopeia SUH-DEVOPS-TEMPLATE 322 2>&1 | python -c "import sys, json; print('github ok:', json.loads(sys.stdin.read())['ok'])"
```
...
echo "---"
for skill in github issue commit report review troubleshoot changelog-deploy; do
  echo "=== $skill ==="
...
# github
cd "$PROJECT_ROOT/skills/github/scripts" && python github_cli.py get-issue Cassiiopeia SUH-DEVOPS-TEMPLATE 322 2>&1 | python -c "import sys, json; print('github get-issue:', json.loads(sys.stdin.read())['ok'])"

# issue
cd "$PROJECT_ROOT/skills/issue/scripts" && python issue_cli.py search-issues Cassiiopeia SUH-DEVOPS-TEMPLATE "skill" 2>&1 | python -c "import sys, json; print('issue search:', json.loads(sys.stdin.read())['ok'])"

# commit
cd "$PROJECT_ROOT/skills/commit/scripts" && python commit_cli.py get-issue-number 2>&1 | python -c "import sys, json; print('commit get-issue-number:', json.loads(sys.stdin.read())['ok'])"

# report
cd "$PROJECT_ROOT/skills/report/scripts" && python report_cli.py get-output-path report 2>&1 | python -c "import sys, json; print('report get-output-path:', json.loads(sys.stdin.read())['ok'])"

# review
cd "$PROJECT_ROOT/skills/review/scripts" && python review_cli.py get-output-path review 2>&1 | python -c "import sys, json; print('review get-output-path:', json.loads(sys.stdin.read())['ok'])"

# troubleshoot
cd "$PROJECT_ROOT/skills/troubleshoot/scripts" && python troubleshoot_cli.py get-output-path troubleshoot 2>&1 | python -c "import sys, json; print('troubleshoot get-output-path:', json.loads(sys.stdin.read())['ok'])"

# changelog-deploy
cd "$PROJECT_ROOT/skills/changelog-deploy/scripts" && python changelog_cli.py deploy-status Cassiiopeia SUH-DEVOPS-TEMPLATE 2>&1 | python -c "import sys, json; print('changelog deploy-status:', json.loads(sys.stdin.read())['ok'])"
```
...
```powershell
$skills = @(
  "github::github_cli.py::get-issue::Cassiiopeia SUH-DEVOPS-TEMPLATE 322",
  "issue::issue_cli.py::search-issues::Cassiiopeia SUH-DEVOPS-TEMPLATE 'skill'",
  "commit::commit_cli.py::get-issue-number::",
  "report::report_cli.py::get-output-path::report",
  "review::review_cli.py::get-output-path::review",
  "troubleshoot::troubleshoot_cli.py::get-output-path::troubleshoot",
  "changelog-deploy::changelog_cli.py::deploy-status::Cassiiopeia SUH-DEVOPS-TEMPLATE"
)
foreach ($s in $skills) {
  $parts = $s -split "::"
...
- `skill-creator/templates/python_cli_script.py` 새 표준 골격
- MCP-style JSON 4필드 (ok/code/summary/next) emit() 헬퍼로 강제

## 검증
- Windows Git Bash MINGW64 ⭕
- WSL Linux bash 5.2 ⭕ (macOS POSIX 프록시)
- 깨졌던 troubleshoot·review `get-output-path` 호출 정상 동작 복구
...
### 깨졌던 호출 복구
- `troubleshoot/SKILL.md` L99-102: `$SCRIPTS_PATH`·`$PYTHON` 미정의 → 표준 5줄로 정상화
- `review/SKILL.md` L88-91: 동일 정상화
EOF

cd "$PROJECT_ROOT/skills/github/scripts" && python github_cli.py add-comment Cassiiopeia SUH-DEVOPS-TEMPLATE 322 /tmp/issue_322_completion.md 2>&1 | python -c "import sys, json; print('comment URL:', json.loads(sys.stdin.read()).get('url'))"
```
