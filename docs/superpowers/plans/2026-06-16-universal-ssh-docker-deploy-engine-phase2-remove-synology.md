...
1. **하위호환 매핑**: `options.synology: true` → `nexus: false` + `secret_backup: false` 매핑 + **안내만 출력**. (이유: 기존 `synology:true` 사용자 대다수는 "배포"를 쓰던 사람이고, 배포는 이제 기본 포함이라 별도 키 불필요. Nexus/Secret-백업까지 자동으로 켜면 원치 않는 워크플로우가 따라붙는다. 보수적으로 둘 다 끄고 "필요하면 --nexus/--secret-backup으로 추가" 안내.)
...
4. **breaking-changes severity**: `warning` (자동 매핑되어 무중단).
...
git commit -m "배포 워크플로우를 확장 가능한 SSH+Docker 엔진으로 재설계 : refactor : synology 폴더 제거 — 배포 워크플로우 루트화·nexus 분리·secret-backup 이동 https://github.com/Cassiiopeia/SUH-DEVOPS-TEMPLATE/issues/388"
```
...
git commit -m "배포 워크플로우를 확장 가능한 SSH+Docker 엔진으로 재설계 : docs : 워크플로우 주석·name의 synology 표현 SSH/서버 중립화 https://github.com/Cassiiopeia/SUH-DEVOPS-TEMPLATE/issues/388"
```
...
git commit -m "배포 워크플로우를 확장 가능한 SSH+Docker 엔진으로 재설계 : feat : integrator(sh) --synology를 --nexus/--secret-backup 게이트로 분리 https://github.com/Cassiiopeia/SUH-DEVOPS-TEMPLATE/issues/388"
```
...
git add template_integrator.sh
git commit -m "배포 워크플로우를 확장 가능한 SSH+Docker 엔진으로 재설계 : feat : integrator(sh) ask_synology_option을 범용 ask_optional_workflow로 교체 https://github.com/Cassiiopeia/SUH-DEVOPS-TEMPLATE/issues/388"
```

---

## Task 5: integrator(.sh) — version.yml 읽기/쓰기 + 하위호환

`read_template_options`(2339-2394)·`save_template_options`(2397-2442) 교체. `options.synology` → `nexus`+`secret_backup`, 구 synology 읽기 시 하위호환 매핑.

**Files:** Modify `template_integrator.sh` (2339-2442)

- [ ] **Step 1: read_template_options의 options 파싱 블록 교체 (2364-2386)**

기존 synology 매치(`return`) 블록을 nexus·secret_backup·구synology 3종 파싱(`continue`)으로:
```bash
        # options 섹션 내부: nexus / secret_backup / (구) synology
        if [ "$in_template" = true ] && [ "$in_options" = true ]; then
            if [[ "$line" =~ ^[[:space:]]+nexus:[[:space:]]*(.+) ]]; then
                local _v=$(echo "${BASH_REMATCH[1]}" | tr -d '"'\''' | xargs)
            ...
            # 하위호환: 구 synology — 배포는 기본 포함이라 opt-in 둘 다 끄고 안내만
            if [[ "$line" =~ ^[[:space:]]+synology:[[:space:]]*(.+) ]]; then
                local _v=$(echo "${BASH_REMATCH[1]}" | tr -d '"'\''' | xargs)
                if [ "$_v" = true ] || [ "$_v" = false ]; then
                    print_info "이전 'synology' 옵션을 감지했습니다 — 배포 워크플로우는 이제 기본 포함이며,"
                    print_info "Nexus/Secret 백업이 필요하면 --nexus/--secret-backup으로 추가하세요."
...
```
> 핵심: 기존은 synology 매치 시 `return`이라 한 키만 읽고 끝났다. 새 코드는 `continue`로 세 키가 한 블록에 공존해도 모두 읽는다. (`tr -d '"'\'''` = 큰따옴표+작은따옴표 제거; bash 3.2 안전 형태로 작성하되 구현 시 기존 `tr -d '"' | tr -d "'"` 2단 파이프 형태를 그대로 써도 무방.)

- [ ] **Step 2: save_template_options 교체 (2411-2439)**

함수 진입부에 기본값 보정 추가: `: "${INCLUDE_NEXUS:=false}"; : "${INCLUDE_SECRET_BACKUP:=false}"`.
synology 쓰기(2412-2421)를 nexus+secret_backup 쓰기 + 구 synology 키 제거로:
```bash
        # nexus 값 업데이트 또는 추가
      ...
      secret_backup: $INCLUDE_SECRET_BACKUP" "$version_file" > "$version_file.tmp" && mv "$version_file.tmp" "$version_file"
        ...
        # 구 synology 키 제거 (deprecated)
        if grep -q "^[[:space:]]*synology:" "$version_file"; then
            grep -v "^[[:space:]]*synology:" "$version_file" > "$version_file.tmp" && mv "$version_file.tmp" "$version_file"
        fi
```
...
- [ ] **Step 4: 하위호환 읽기 검증 (synology:true)**
...
```
Expected: `<<NEXUS=false SECRET=false>>` + "이전 'synology' 옵션을 감지" 안내.

- [ ] **Step 5: 쓰기 검증 (새 키 기록 + synology 제거)**
...
```
Expected: `nexus: true`, `secret_backup: false` 존재, `synology:` 키 사라짐.

- [ ] **Step 6: Commit**

```bash
git add template_integrator.sh
git commit -m "배포 워크플로우를 확장 가능한 SSH+Docker 엔진으로 재설계 : feat : integrator(sh) version.yml options를 nexus/secret_backup로 전환 + synology 하위호환 매핑 https://github.com/Cassiiopeia/SUH-DEVOPS-TEMPLATE/issues/388"
```
...
```bash
git add template_integrator.sh
git commit -m "배포 워크플로우를 확장 가능한 SSH+Docker 엔진으로 재설계 : feat : integrator(sh) 복사·메뉴·요약을 nexus/secret-backup으로 전환, synology 완전 제거 https://github.com/Cassiiopeia/SUH-DEVOPS-TEMPLATE/issues/388"
```
...
- [ ] **Step 5: version.yml 읽기/쓰기 (1937-2042)** — sh Task 5 로직을 PowerShell 정규식으로. `synology:` 읽기→하위호환 매핑(둘 다 $false + 안내), `nexus:`/`secret_backup:` 읽기·쓰기, 구 `synology` 라인 제거(`-replace` 또는 `Where-Object`).
...
- [ ] **Step 10: 함수 단위 입력 주입 (하위호환 매핑)** — CLAUDE.md "함수만 떼어내 입력 주입" 패턴. `Read-TemplateOptions` 상당 로직만 잘라 `synology: true` version.yml 주입 → `IncludeNexus=$false`·`IncludeSecretBackup=$false` 매핑 확인. (QEMU 함수호출 크래시 시 인라인 실행으로 로직 정상 여부 구별 — CLAUDE.md 절차.)
...
git commit -m "배포 워크플로우를 확장 가능한 SSH+Docker 엔진으로 재설계 : feat : integrator(ps1) synology 전면 제거 — nexus/secret-backup 게이트로 전환 https://github.com/Cassiiopeia/SUH-DEVOPS-TEMPLATE/issues/388"
```
...
git commit -m "배포 워크플로우를 확장 가능한 SSH+Docker 엔진으로 재설계 : chore : version.yml options 전환 + breaking-changes 폴더구조 변경 안내 등록 https://github.com/Cassiiopeia/SUH-DEVOPS-TEMPLATE/issues/388"
```
...
git commit -m "배포 워크플로우를 확장 가능한 SSH+Docker 엔진으로 재설계 : docs : 배포 가이드 범용화 개명 + README/CLAUDE.md synology 표현 정리 https://github.com/Cassiiopeia/SUH-DEVOPS-TEMPLATE/issues/388"
```
...
**1. Spec coverage:**
- spec §3 폴더재편 → Task 1 ✅ / §4 워크플로우 내부 주석·name → Task 2 ✅ (인증분기·SUDO는 Phase 1 완료) / §5 integrator → Task 3-7 ✅ / §5.4 version.yml+하위호환 → Task 5,8 ✅ / §6 breaking-changes → Task 8 ✅ / §7 문서 → Task 9 ✅ / §8 CLAUDE.md → Task 9 ✅ / §9 검증 → 각 Task + Task 10 ✅. spec §11 미해결 5건 → 본 plan 상단에서 전부 확정 ✅.
