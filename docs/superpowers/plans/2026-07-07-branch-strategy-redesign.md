...
### Task 6: changelog-deploy 스킬 · changelog_cli.py 개정

**Files:**
- Modify: `skills/changelog-deploy/scripts/changelog_cli.py` (404행)
- Modify: `skills/changelog-deploy/SKILL.md` (브랜치 참조 전면)
...
`grep -n "deploy\|main" skills/changelog-deploy/SKILL.md`로 전 행을 나열하고, 남은 서술을 다음 규칙으로 교체한다:
...
- 스킬/파일/서브커맨드 이름(`changelog-deploy`, `deploy-status`, `changelog_deploy`)과 config 키는 **절대 변경 금지**

- [ ] **Step 5: 검증 + 커밋**

```bash
grep -n "origin/deploy\|--base deploy\|\"deploy\")" skills/changelog-deploy/SKILL.md
```

Expected: 출력 없음.

```bash
git add skills/changelog-deploy
...
**Files:**
- Modify: `skills/github/SKILL.md` (156-168행)
...
- [ ] **Step 1: github SKILL.md 릴리스 노트 섹션 교체**
...
```bash
git add skills/github/SKILL.md skills/references/common-rules.md CLAUDE.md
...
develop에서 사소한 변경(예: docs) 커밋 → `/projectops:changelog-deploy`로 develop→main 릴리스 PR 생성 → 확인 항목:
