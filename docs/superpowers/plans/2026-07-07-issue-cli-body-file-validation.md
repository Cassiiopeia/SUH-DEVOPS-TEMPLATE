...
`skills/issue/scripts/issue_cli.py` 파일의 `cmd_create_issue` 함수를 다음과 같이 수정하여 파일 미존재 시 즉시 에러 응답을 하고, 성공 시 `body_length`를 함께 `emit` 하도록 합니다.

```python
# skills/issue/scripts/issue_cli.py 내 수정 내용
...
```bash
git add skills/issue/scripts/issue_cli.py scripts/tests/test_cli_body_file.py
...
`skills/issue/scripts/issue_cli.py`에서 `cmd_update_issue`와 `build_parser` 안의 `p_ui` 파트를 수정합니다.

```python
# skills/issue/scripts/issue_cli.py 내 수정 내용 (cmd_update_issue 및 build_parser)
...
`skills/github/scripts/github_cli.py`의 `cmd_update_issue`와 `build_parser` 부분을 수정합니다.

```python
# skills/github/scripts/github_cli.py 내 수정 내용
...
```bash
git add skills/github/scripts/github_cli.py scripts/tests/test_cli_body_file.py
