# changelog-deploy 릴리스 노트 임시파일 위치 버그 수정 설계

- 날짜: 2026-06-11
- 상태: 설계 승인됨 (구현 전)
- 범위: `changelog-deploy` 스킬의 릴리스 노트 임시파일 버그만 (다른 스킬·기능 변경 없음)

---

## 1. 문제 (근본 원인)

`changelog-deploy` 스킬은 릴리스 노트를 임시파일에 쓰고, deploy PR 본문에 넣은 뒤
삭제한다. 그런데 **만드는 경로와 지우는 경로가 다르다.**

- **만들기**: agent가 Write 도구로 `$PROJECT_ROOT/scripts/_release_notes.md`
  (레포 내부, git이 추적하는 폴더)
- **지우기**: `cd "$PROJECT_ROOT/skills/changelog-deploy/scripts"` 한 cwd에서
  `rm -f _release_notes.md` → 실제로는 `skills/changelog-deploy/scripts/_release_notes.md`를
...
### 3.1 `skills/changelog-deploy/SKILL.md` (3지점)
...
### 3.2 `skills/changelog-deploy/scripts/changelog_cli.py` (정합성)
...
| 파일 | 작업 |
|---|---|
| `skills/changelog-deploy/SKILL.md` | 5단계·6단계·fix4·fix5의 임시파일 경로를 `~/.suh-template/tmp/`로 통일 |
| `skills/changelog-deploy/scripts/changelog_cli.py` | `_resolve_body_file` 후보에 홈 tmp 추가 + 주석·에러메시지 갱신 |
