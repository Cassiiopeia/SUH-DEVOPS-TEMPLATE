...
### 9-2. 값 주입 방식 — 런타임 읽기 폐기, 통합 시점 1회 치환 (사용자 피드백 반영)

> **설계 변경 이유(사용자 지적)**: 런타임에 `version_manager.sh get-path`로 매번 읽으면 ① version.yml이 망가지면 배포가 죽고 ② 사용자가 흐름을 모르며 ③ 직관적이지 않다. → **값을 워크플로우 상단 `env`에 박고, 사람이 보고 고칠 수 있게** 한다. 런타임은 version.yml을 읽지 않는다.

- 워크플로우 상단에 사람이 읽고 수정 가능한 env 토큰을 둔다:
  ```yaml
  env:
    FLUTTER_ROOT: "."   # @wizard flutter-root
  ```
- **integrator/마법사 통합 시점에 자동 치환** — 이미 존재하는 `@wizard` 토큰 치환 엔진 활용(`template_integrator.sh: configure_workflow_env`, `template_integrator.ps1` 대응). 사용자는 아무 입력 안 해도 됨(auto). 치환 후 마커는 `# @wizard set (직접 수정 가능)`으로 바뀌어 수동 수정 안내.
- **`@wizard flutter-root` 마커를 신규 추가**한다. 동작은 기존 `# @wizard paths-anchor`(2983~2990행)와 동일 패턴: `get_path_for_type "flutter"` 반환값(단일레포 `.` / 모노레포 `app`)을 그대로 치환.
  - 주의: 기존 `# @wizard auto-find`는 Spring `application*.yml` 탐색에 하드코딩(2923행)돼 있어 Flutter엔 부적합. **재사용 말고 전용 `flutter-root` 마커를 추가**한다. `paths-anchor`가 이미 `project_paths` 값을 그대로 박는 검증된 전례이므로 같은 방식.
  - `get_path_for_type`은 통합 시점에 `resolve_project_paths`(1195~)가 이미 채워둔 CSV(=`project_paths` 확정값)를 읽으므로, 별도 find 재탐색 불필요.
- **integrator를 안 거친 경우**(템플릿 레포 자체에서 돌거나 사용자가 수동 배치): 토큰 기본값 `.`이 남아 단일레포로 동작 → 안전 폴백.
- `.ps1` 동등성: `configure_workflow_env`에 해당하는 PowerShell 치환 로직에도 `flutter-root` 마커 처리를 동일하게 추가.
...
- **치환 엔진**: `template_integrator.sh`/`.ps1`에 `flutter-root` 마커 추가 후, CLAUDE.md의 integrator 검증법(Docker PowerShell 파서 / expect TTY)으로 문법·동작 확인. `bash -n`·`Parser::ParseFile` 통과 + 모노레포 픽스처에서 치환 결과 대조.
- **런타임 version.yml 미의존 확인**: 워크플로우 run 로그에 `version_manager.sh get-path` 호출이 없어야 함(값은 env에 박혀 있음).

### 9-5. 모노레포 대응 비범위
- `version_manager.sh`에 `get-path` CLI 서브커맨드 추가는 **하지 않는다**(런타임 읽기 폐기로 불필요해짐). 경로 확정은 integrator의 `resolve_project_paths`+`@wizard flutter-root`로 충분.
