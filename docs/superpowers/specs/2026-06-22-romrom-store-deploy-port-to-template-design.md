...
**(B) 모노레포 경로 대응 (passQL이 증명한 결함)** — Flutter 루트가 레포 루트의 서브폴더(예: `app/`)인 모노레포에서, **마법사는 `PROJECT_PATH` 인자로 올바른 위치에 깔지만 CICD 워크플로우가 `cd ios`/`working-directory: ios`로 레포 루트 기준 하드코딩이라 전부 실패**한다. CICD가 `version.yml: project_paths.flutter`를 읽어 Flutter 루트를 동적으로 잡게 한다. §9 참조.
...
- 단일 진실 소스는 이미 존재: `version.yml: project_paths.flutter`(키 없으면 루트 기준). `version_manager.sh: get_type_path()`가 이 값을 읽지만 **CLI로 노출 안 됨.**

### 9-2. 변경 범위 (전수 조사 결과)
- iOS 워크플로우 경로 의존 지점 **23개**, Android **24개** (총 47개).
- 두 워크플로우 모두 **`defaults: run: working-directory` 미설정** — `cd`가 step마다 흩어짐.
- 버전 읽기는 이미 `version_manager.sh get`/`get-code`로 간접 처리(직접 yq/grep 없음).

### 9-3. 채택 방식: job `defaults.run.working-directory` 중심
1. **`version_manager.sh`에 `get-path <type>` 서브커맨드 추가** — 기존 `get_type_path()`를 CLI로 노출(키 없으면 `.` 반환, legacy 동작 유지). 단일 소스 재사용.
2. 각 워크플로우 `prepare-build` 류 job에서 `FLUTTER_ROOT=$(./.github/scripts/version_manager.sh get-path flutter)`를 구해 job output 또는 env로 전달.
3. **빌드/배포 job에 `defaults: run: working-directory: ${{ env.FLUTTER_ROOT }}`(또는 needs로 받은 output) 한 줄** 설정 → 그 job의 모든 `run:` step이 Flutter 루트에서 돈다. 이로써 `cd ios`(→ job 안에서 `cd ios`는 `$FLUTTER_ROOT/ios`로 자동 해석), `mkdir android/...`, `cat > android/key.properties` 등 **run 내부 경로 47개 중 대부분이 자동 해결**.
4. **`run:`이 아니라 자동 적용 안 되는 곳만 개별 변수화**:
   - `actions/upload-artifact`·`download-artifact`의 `path:` (working-directory 영향 안 받음) → `${{ needs.prepare.outputs.flutter_root }}/build/...`
   - step 레벨 `working-directory:` 값 자체
   - `$GITHUB_WORKSPACE/ios/...` 같은 **절대경로**로 쓴 곳(예: iOS `find $GITHUB_WORKSPACE/ios/build/ipa`) → `$GITHUB_WORKSPACE/$FLUTTER_ROOT/ios/...`
   - `git add ios/`처럼 레포 루트에서 도는 게 맞는 것은 working-directory와 별개로 점검
5. 기본값 `.`이면 **현재 단일레포 동작 100% 보존**(RomRom 포함). 모노레포는 `project_paths.flutter` 값만 맞으면 동작.

### 9-4. 검증
- **단일레포 회귀**: `project_paths` 없는 상태에서 `FLUTTER_ROOT="."`로 기존과 동일 경로가 나오는지(`./ios`=`ios`) 확인. RomRom 동형.
- **모노레포**: passQL(`project_paths.flutter: "app"`)에서 `get-path flutter`가 `app`을 반환하고, `defaults working-directory: app`으로 `cd ios`가 `app/ios`를 가리키는지, artifact path가 `app/build/...`로 잡히는지 확인.
- `version_manager.sh get-path` 단위 동작은 `bash`로 직접 호출해 검증(추가 서브커맨드라 mac/win 양쪽 yq 의존성 확인).
...
- `project_paths`에 키가 여러 개(멀티타입) 있어도 이번엔 `flutter`만 사용.
