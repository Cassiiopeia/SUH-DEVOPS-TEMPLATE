...
> **이 절은 선행 스펙 `2026-06-22-wizard-marker-redesign-design.md`에 의존한다.** 마커 시스템 전면 재설계(`# @wizard ask:<기본값>`/`auto:<resolver>` 문법 + labels.yml + resolver 레지스트리)가 먼저 완료된 전제 위에 Flutter 경로만 얹는다.
...
2. **마커에 action·resolver, 워크플로우만 봐도 동작이 보임** — `FLUTTER_ROOT`는 `auto:`(안 물음)라 labels.yml도 불필요.
...
- **integrator 미경유**: 마커만 있고 치환 안 됨 → env 폴백값 `"."`로 단일레포 동작(안전 폴백).
...
- **`flutter-root` resolver**: 워크플로우에 `# @wizard auto:flutter-root` 추가 후, 단일레포에서 `.`·passQL에서 `app`을 반환하는지(`get_path_for_type "flutter"` 위임). `.sh`/`.ps1` 동일.
...
### 9-5. 모노레포 대응 비범위
- 마커 문법(ask:/auto:)·labels.yml·resolver 골격과 기존 마커 마이그레이션은 **선행 마커 스펙** 소관(이 스펙은 `flutter-root` 적용만).
