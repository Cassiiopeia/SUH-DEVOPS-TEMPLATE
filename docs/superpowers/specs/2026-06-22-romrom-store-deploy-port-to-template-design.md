...
### 9-2. 값 주입 방식 — 마커 재설계 스펙에 위임 (선행 의존)

> **이 절은 선행 스펙 `2026-06-22-wizard-marker-redesign-design.md`에 의존한다.** 마커 시스템 전면 재설계(`# wizard` 표식 + `fields.yml` 명세 + resolver 레지스트리)가 먼저 완료된 전제 위에 Flutter 경로만 얹는다.

**설계 원칙(사용자 피드백 누적)**:
1. **런타임 읽기 폐기** — `version_manager.sh get-path`로 매번 읽으면 version.yml이 망가질 때 배포가 죽고, 사용자가 흐름을 모르며, 직관적이지 않다. → 값을 워크플로우 `env`에 박고 **통합 시점에 1회 치환**, 런타임은 version.yml을 읽지 않는다.
2. **마커는 표식만, 명세는 fields.yml** — 워크플로우엔 `# wizard` 표식 한 줄, 무엇으로 채울지는 `.github/wizard/fields.yml`이 결정(파싱 단순·따옴표 충돌 0·확장은 fields.yml 한 블록 + resolver 함수 1개).

**Flutter가 이 위에 추가하는 것 (전부):**
- 워크플로우 env에 표식 추가:
  ```yaml
  env:
    FLUTTER_ROOT: "."   # wizard
...
- `fields.yml`에 한 블록:
  ...
  ```
- resolver 함수 추가: `.sh` `resolve_flutter_root()` / `.ps1` `Resolve-FlutterRoot` → `get_path_for_type "flutter"`(통합 시점 `resolve_project_paths`로 채워진 값, `.` 또는 `app`) 반환.
- 치환 후 `# wizard` 주석은 삭제(마커 스펙 §3-4) → 사용자 워크플로우엔 `FLUTTER_ROOT: "app"`만 남음.
- **integrator 미경유**: 표식만 있고 치환 안 됨 → env 폴백값 `"."`로 단일레포 동작(안전 폴백).
- `.sh`/`.ps1` resolver 동등성은 마커 스펙 §5 검증으로 보장.
...
### 9-4. 검증 (Flutter 범위)
> 마커 엔진·resolver·Spring 마이그레이션 회귀 검증은 선행 마커 스펙 §5 소관. 여기선 Flutter 경로 적용만.
- **`flutter-root` resolver**: `fields.yml`에 `FLUTTER_ROOT: {resolve: flutter-root}` 추가 후, 단일레포에서 `.`·passQL에서 `app`을 반환하는지(`get_path_for_type "flutter"` 위임). `.sh`/`.ps1` 동일.
- **단일레포 회귀**: `FLUTTER_ROOT="."`일 때 기존과 동일 경로(`./ios`=`ios`, `defaults working-directory: .`)가 되는지 확인. RomRom 동형.
- **모노레포**: passQL(`project_paths.flutter: "app"`)을 integrator로 통합 시 워크플로우 env가 `FLUTTER_ROOT: "app"`으로 치환되고 `# wizard` 주석이 삭제되는지, `defaults working-directory: app`으로 `cd ios`가 `app/ios`를 가리키는지, artifact path가 `app/build/...`로 잡히는지 확인.
- **런타임 version.yml 미의존**: 워크플로우 run 로그에 version.yml 읽기 호출이 없어야 함(값은 env에 박혀 있음).

### 9-5. 모노레포 대응 비범위
- 마커 엔진·`fields.yml`·resolver 골격과 기존 마커 마이그레이션은 **선행 마커 스펙** 소관(이 스펙은 `flutter-root` 적용만).
