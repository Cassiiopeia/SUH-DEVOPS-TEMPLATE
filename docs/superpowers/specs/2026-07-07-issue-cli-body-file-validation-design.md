...
현재 `suh-issue` 및 `suh-github` 스킬에서 사용하는 CLI 도구들(`issue_cli.py`, `github_cli.py`)은 이슈 본문 생성 실패 시 빈 본문(`""`)으로 조용히 처리해 버립니다(Silent Fallback). 이로 인해 특히 Windows 한글 경로 깨짐 등의 환경적 요인으로 본문 파일 로드에 실패했을 때, AI나 사용자에게 아무런 에러 없이 빈 이슈가 등록됩니다. 
...
### 4.1 `skills/suh-issue/scripts/issue_cli.py`
...
### 4.2 `skills/suh-github/scripts/github_cli.py`
