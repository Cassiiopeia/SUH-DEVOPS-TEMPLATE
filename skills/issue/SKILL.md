...
3. **Config 확인** — `references/config-rules.md` §2~5 절차를 따른다.
   ...
   > ⚠️ **config는 탐색하지 않는다.** config.json은 고정 경로 `{HOME}/.suh-template/config/config.json` **한 곳에만** 있다. Read tool로 이 경로를 바로 읽는다. `ls`·`find`·glob으로 찾지 마라 — 아래 스크립트(`issue_cli.py`) 위치 탐색에 쓰는 `ls ~/.claude/plugins/cache/...` 패턴은 **스크립트 전용**이며, config.json은 그 캐시 안에 없다. 캐시를 뒤지면 엉뚱한 파일을 잡거나 "config 없음"으로 오판해 사용자에게 PAT를 다시 묻게 된다.
