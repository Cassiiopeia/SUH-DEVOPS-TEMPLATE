...
3. **Config 확인** — `references/config-rules.md` §2~5 절차를 따른다 (`skill_id = issue`).

   파일이 존재하면 → `github_pat`, `github_repos` 추출. `github_repos` 중 `default: true`인 repo 사용. repo가 여러 개면 번호를 매겨 선택하게 한다.

   파일이 없으면 → 아래 항목을 하나씩 수집 후 저장:
   - GitHub PAT 토큰 (repo 권한 필요. 발급: GitHub > Settings > Developer settings > Personal access tokens)
...
config에서 읽은 `github_pat`을 사용해 GitHub API를 직접 호출한다:
