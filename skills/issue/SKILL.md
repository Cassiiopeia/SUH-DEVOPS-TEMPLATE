...
2. **프로젝트 루트, PYTHON, SCRIPTS_PATH 변수 설정**:
   ...
   echo "PROJECT_ROOT=$PROJECT_ROOT SCRIPTS_PATH=$SCRIPTS_PATH"
   ```
...
```bash
PROJECT_ROOT=$(git rev-parse --show-toplevel 2>/dev/null || pwd)
PYTHONPATH="$SCRIPTS_PATH" $PYTHON -m suh_template.cli get-output-path issue --title "{이슈 제목}"
```
...
GITHUB_PAT=$(PYTHONPATH="$SCRIPTS_PATH" $PYTHON -m suh_template.cli config-get issue github_pat) \
  PYTHONPATH="$SCRIPTS_PATH" $PYTHON -m suh_template.cli create-issue {owner} {repo} "{제목}" /tmp/issue_body.md "{라벨}"
```
...
```bash
PYTHONPATH="$SCRIPTS_PATH" $PYTHON -m suh_template.cli create-branch-name "{이슈 제목}" {이슈번호}
```

### 7단계: 커밋 템플릿 계산

```bash
PYTHONPATH="$SCRIPTS_PATH" $PYTHON -m suh_template.cli get-commit-template "{이슈 제목}" "{이슈 URL}"
```
