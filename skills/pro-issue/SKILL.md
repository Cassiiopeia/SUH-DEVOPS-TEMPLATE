---
name: pro-issue
description: "Issue Mode - GitHub 이슈 작성 전문가. 사용자의 대략적인 설명을 받아 GitHub 이슈 템플릿(.github/ISSUE_TEMPLATE)에 맞는 제목·본문을 자동 작성하고 로컬 파일로 저장한 뒤 사용자 확인 후 GitHub에 등록한다. '이슈 만들어줘', '이슈 올려줘', '이슈 등록', '이슈 작성', '버그 이슈', '버그 리포트', '기능 요청 이슈', 'QA 요청 이슈', '디자인 요청 이슈', '/issue' 등 GitHub 이슈를 새로 만드는 모든 요청에 반드시 이 스킬을 사용한다. 이 스킬을 거치지 않고 curl·GitHub API로 이슈를 직접 생성하는 것은 금지 — 템플릿 규격·제목 prefix·중복검사·로컬 파일 저장 절차를 우회하기 때문이다. 이슈 생성이 커밋·푸시·보고서 등 다른 작업과 묶인 복합 요청이라도, 이슈 생성 단계만큼은 반드시 이 스킬로 처리한 뒤 나머지를 진행한다. /issue 호출 시 사용."
---
...
3. **Config 확인** — `references/config-rules.md` §2~5 절차를 따른다.
   ...
   > ⚠️ **config는 탐색하지 않는다.** config.json은 고정 경로 `{HOME}/.projectops/config/config.json` **한 곳에만** 있다. Read tool로 이 경로를 바로 읽는다. `ls`·`find`·glob으로 찾지 마라 — 아래 스크립트(`issue_cli.py`) 위치 탐색에 쓰는 `ls ~/.claude/plugins/cache/...` 패턴은 **스크립트 전용**이며, config.json은 그 캐시 안에 없다. 캐시를 뒤지면 엉뚱한 파일을 잡거나 "config 없음"으로 오판해 사용자에게 PAT를 다시 묻게 된다.
...
6. **담당자(assignee) 결정** — 위 3번에서 읽은 `config.json`에서 이슈 담당자를 결정한다. **auto_approve와 똑같은 "글로벌 + 레포별 오버라이드 + 첫 실행만 질문" 패턴**을 따른다.

   해석 우선순위 (위→아래로 검사, 먼저 발견되는 값 채택):
   1. `github.repos[]` 중 `owner == 현 OWNER && repo == 현 REPO`인 항목의 `issue.assignee` (레포별 오버라이드)
   2. `github.default_assignee` (글로벌 기본값)
   3. 어디에도 없으면 **미설정** (첫 실행 케이스 — 5단계에서 한 번만 질문)

   판정 결과를 두 값으로 **기억**한다:
   - `ASSIGNEE` — 담당자 GitHub 사용자명 (또는 미설정)
   - `ASSIGNEE_HAS_KEY` — `true`면 위 1·2에서 값 발견. `false`면 첫 실행 케이스

   > **핵심 — 사용자 무간섭**: `ASSIGNEE`가 정해져 있으면(대부분 `default_assignee`로) **묻지 않고 자동 적용**한다. 4단계 승인 화면에 "담당자: {ASSIGNEE}"를 **표시만** 한다. `ASSIGNEE_HAS_KEY=false`(config에 담당자가 전혀 없을 때)에만 5단계에서 한 번 질문하고 그 답을 config에 저장 → 이후 영구 자동.
...
일반 키보드로 바로 칠 수 없는 특수 문장부호는 "AI가 만든 티"가 나므로 제목에서 쓰지 않는다. 부연이나 나열은 키보드에 있는 문자로 대체한다.

- **em dash(`—`), en dash(`–`) 사용 금지.** "주제 — 부연"처럼 줄표로 제목을 압축하지 않는다.
  - 부연이 필요하면 **콜론(`:`), 쉼표(`,`), 괄호(`()`)** 로 대체하거나, 아예 한 구절로 자연스럽게 이어 쓴다.
  - ❌ `스킬 네임스페이스 리브랜딩 — suh-* 중립화`
  - ✅ `스킬 네임스페이스 리브랜딩: suh-* 중립화`
  - ✅ `suh-* 중립화를 위한 스킬 네임스페이스 리브랜딩`
- **가운뎃점(`·`, middle dot) 사용 금지.** 항목을 나열할 때 `A·B·C`처럼 붙이지 않는다.
  - **쉼표(`,`)** 또는 **슬래시(`/`)** 로 대체한다.
  - ❌ `스킬 경로 옛 이름·워크플로우명·IDE 표 누락 수정`
  - ✅ `스킬 경로 옛 이름, 워크플로우명, IDE 표 누락 수정`
- 일반 하이픈(`-`)은 파일명, 버전(`integrator.sh/.ps1`, `v4.2.0`) 등 원래 표기에 필요한 경우만 그대로 둔다.
...
**인라인 Python 작성 금지.** 재사용 스크립트 `skills/pro-issue/scripts/issue_cli.py`의 `search-issues`를 호출한다. keyword는 마지막 인자로 그대로 넘기며(공백 포함 가능, 따옴표로 감쌈), `issue_cli`가 내부에서 URL 인코딩한다. **PAT는 `issue_cli`가 config.json에서 자동 로드하므로 `GITHUB_PAT=`는 생략 가능**하다(환경변수가 있으면 우선 사용).

```bash
PROJECT_ROOT=$(git rev-parse --show-toplevel 2>/dev/null || pwd)
PYTHON=$(for _py in python3 python; do _path=$(command -v "$_py" 2>/dev/null) || continue; "$_path" -c "import sys; sys.exit(0)" 2>/dev/null && echo "$_path" && break; done)
[ -z "$PYTHON" ] && { echo "Python not found"; exit 1; }
SCRIPTS=$(ls -d ~/.claude/plugins/cache/*/projectops/*/skills/pro-issue/scripts 2>/dev/null | sort -V | tail -1); [ -z "$SCRIPTS" ] && SCRIPTS="$PROJECT_ROOT/skills/pro-issue/scripts"; cd "$SCRIPTS" || exit 1
...
#### C-2. 담당자 첫 설정 (auto_approve와 별개로, `ASSIGNEE_HAS_KEY == false`일 때만, 한 번)

[시작 전 §6]에서 `ASSIGNEE_HAS_KEY == false`(config에 담당자가 전혀 없음)면, 4-1단계로 가기 **전에** 한 번만 묻는다. (config에 담당자가 이미 있으면 이 분기는 건너뛰고 묻지 않는다 — 자동 적용.)
...
- 사용자명 입력 → `github.default_assignee`에 저장 (모든 레포 공통 기본 담당자). 이번 이슈부터 `ASSIGNEE`로 사용
- "없음" → `github.default_assignee`를 빈 문자열로 저장 (다음부터 묻지 않음). 담당자 없이 진행

> 이후 이 사용자는 다시 담당자를 묻지 않는다. 특정 레포만 다른 담당자를 원하면 "이 레포 담당자는 {사용자명}으로"처럼 자연어로 말하면 agent가 `repos[].issue.assignee`에 저장한다.
...
**인라인 Python 작성 금지.** 재사용 스크립트 `skills/pro-issue/scripts/issue_cli.py`의 `create-issue`를 호출한다. body는 로컬에 저장한 이슈 `.md` 파일(템플릿 섹션만 담긴 본문)을 `body_file`로 전달하므로 줄바꿈·이모지·한국어가 안전하게 보존된다. **PAT는 `issue_cli`가 config.json에서 자동 로드하므로 `GITHUB_PAT=`는 생략 가능**하다(환경변수가 있으면 우선 사용).
...
SCRIPTS=$(ls -d ~/.claude/plugins/cache/*/projectops/*/skills/pro-issue/scripts 2>/dev/null | sort -V | tail -1); [ -z "$SCRIPTS" ] && SCRIPTS="$PROJECT_ROOT/skills/pro-issue/scripts"; cd "$SCRIPTS" || exit 1
PYTHONIOENCODING=utf-8 "$PYTHON" issue_cli.py \
  create-issue {owner} {repo} "{제목}" "{이슈 본문 .md 파일 절대경로}" "{라벨 csv}" --assignees "{ASSIGNEE}"
```

`--assignees`에는 [시작 전 §6]에서 결정한 `ASSIGNEE`를 그대로 넘긴다. 미설정이면 생략하거나 빈 값 — 담당자 없이 생성된다. 요청한 담당자가 반영되지 않으면 출력에 `assignee_warning`이 들어오며, 이슈는 정상 생성된 것이므로 중단하지 않고 그 경고만 자연어로 전달한다.

출력은 JSON: `{"number": ..., "url": ..., "title": ..., "assignees": [...]}`. `number`와 `url`을 추출한다.
