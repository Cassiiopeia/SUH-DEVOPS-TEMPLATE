...
- **`suh_command` 서브커맨드 호출 시** → PAT를 아예 신경 쓰지 않는다.
  `suh_command`가 `GITHUB_PAT` 환경변수 → 없으면 `config.json`에서 위 우선순위로 **자동 로드**한다.
  `GITHUB_PAT=`를 안 붙여도 동작한다.

- **curl로 직접 호출 시** → 표준 추출기 `scripts/suh_template/get_pat.py`를 쓴다.
  ...
  `get_pat.py`는 위 우선순위(repo별 `pat` → `global_pat`)를 그대로 구현하므로
