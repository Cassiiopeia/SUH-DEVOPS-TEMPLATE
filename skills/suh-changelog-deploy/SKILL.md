...
> **왜 필요한가**: 앱스토어/플레이스토어 심사 자동 제출이 켜진 레포에서는 `deploy`가 "내부 배포"가 아니라 **사용자 대면 출시**가 된다. 릴리스 노트가 그대로 스토어 "이번 업데이트" 출시노트가 되어 심사에 들어간다. 따라서 이 레포가 앱 심사에 직결되는지 먼저 인지하고, 그렇다면 릴리스 노트를 더 신중히 다룬다. **백엔드 레포(spring/python 등)는 아무 영향 없이 조용히 통과한다.**
