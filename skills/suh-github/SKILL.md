...
PAT를 쓰는 방법은 **두 경로**가 있고, 절대 인라인 Python으로 PAT를 즉흥 추출하지 않는다:

1. **`suh_command` 서브커맨드로 호출할 때 (이슈·PR·Actions 등)** → PAT를 신경 쓸 필요 없다.
   `suh_command`가 `GITHUB_PAT` 환경변수 → 없으면 `config.json`(`github.global_pat`, repo별 `pat` 우선) 순으로 **자동 로드**한다.
   따라서 `GITHUB_PAT=` 를 안 붙여도 동작한다.

2. **curl로 직접 호출할 때 (explore·secret·README 등)** → 아래 표준 스니펫으로 PAT를 변수에 담는다.
   `references/config-rules.md` §3의 PAT 우선순위(repo별 `pat` → `global_pat`)를 그대로 구현한 것이다.

   ```bash
   PYTHON=$(for _py in python3 python; do _path=$(command -v "$_py" 2>/dev/null) || continue; "$_path" -c "import sys; sys.exit(0)" 2>/dev/null && echo "$_path" && break; done)
   # owner/repo는 아래 'Repo 자동 감지'에서 결정한 값으로 치환
   PAT=$("$PYTHON" "$PROJECT_ROOT/scripts/suh_template/get_pat.py" {owner} {repo})
   # PAT가 비면 → config.json 없음. /issue 스킬로 PAT 등록 안내
   ```
...
```bash
PYTHON=$(for _py in python3 python; do _path=$(command -v "$_py" 2>/dev/null) || continue; "$_path" -c "import sys; sys.exit(0)" 2>/dev/null && echo "$_path" && break; done)
GITHUB_PAT="$PAT" PYTHONIOENCODING=utf-8 $PYTHON - <<'EOF'
import urllib.request, json
import os
pat = os.environ["GITHUB_PAT"]
...
> **Windows 주의**: `cd "$PROJECT_ROOT/scripts"` 후 `-m suh_template.suh_command`로 실행한다. `/tmp`·`curl|python` stdin pipe·heredoc 보간은 사용하지 않는다 (Windows Git Bash에서 깨짐). 인자는 모두 명령행/환경변수로 전달한다.
...
2. owner 명시 ("TEAM-ROMROM", "Cassiiopeia" 등) → 해당 owner 사용. **이 경우에만** 타입 판별 실행:
   ```bash
   curl -s -H "Authorization: token $PAT" "https://api.github.com/users/{owner}" \
