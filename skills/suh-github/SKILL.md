...
> **Windows 주의**: `cd "$PROJECT_ROOT/skills/suh-github/scripts"` 후 `github_cli.py`로 실행한다. 임시 파일 파싱·curl 파이프 Python·heredoc 보간은 사용하지 않는다 (Windows Git Bash에서 깨짐). 인자는 모두 명령행/환경변수로 전달한다.
