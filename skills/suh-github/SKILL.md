...
GitHub API 호출은 재사용 스크립트 `scripts/suh_template/suh_command.py`로만 수행한다. PAT는 `suh_command`가 `GITHUB_PAT` 환경변수 → `config.json`(`github.global_pat`, repo별 `pat` 우선) 순으로 자동 로드하므로 호출부에서 직접 추출하지 않는다.
...
PYTHONIOENCODING=utf-8 "$PYTHON" -m suh_template.suh_command get-issue {owner} {repo} {이슈번호}
PYTHONIOENCODING=utf-8 "$PYTHON" -m suh_template.suh_command get-issue {owner} {repo} {이슈번호} --with-comments
```
...
```bash
PYTHONIOENCODING=utf-8 "$PYTHON" -m suh_template.suh_command get-issues {owner} {repo} 712 707 715
```
...
PYTHON=$(for _py in python3 python; do _path=$(command -v "$_py" 2>/dev/null) || continue; "$_path" -c "import sys; sys.exit(0)" 2>/dev/null && echo "$_path" && break; done)
cd "$PROJECT_ROOT/scripts"
PYTHONIOENCODING=utf-8 "$PYTHON" -m suh_template.suh_command update-issue {owner} {repo} {이슈번호} \
...
PYTHON=$(for _py in python3 python; do _path=$(command -v "$_py" 2>/dev/null) || continue; "$_path" -c "import sys; sys.exit(0)" 2>/dev/null && echo "$_path" && break; done)
cd "$PROJECT_ROOT/scripts"
PYTHONIOENCODING=utf-8 "$PYTHON" -m suh_template.suh_command add-comment {owner} {repo} {이슈번호} "{댓글 본문 파일 경로}"
```
...
PYTHON=$(for _py in python3 python; do _path=$(command -v "$_py" 2>/dev/null) || continue; "$_path" -c "import sys; sys.exit(0)" 2>/dev/null && echo "$_path" && break; done)
cd "$PROJECT_ROOT/scripts"
PYTHONIOENCODING=utf-8 "$PYTHON" -m suh_template.suh_command create-pr {owner} {repo} "{제목}" "{PR 본문 파일 경로}" "{owner}:{head_branch}" main
```
...
PYTHON=$(for _py in python3 python; do _path=$(command -v "$_py" 2>/dev/null) || continue; "$_path" -c "import sys; sys.exit(0)" 2>/dev/null && echo "$_path" && break; done)
cd "$PROJECT_ROOT/scripts"
PYTHONIOENCODING=utf-8 "$PYTHON" -m suh_template.suh_command list-prs {owner} {repo} --state open
...
PYTHON=$(for _py in python3 python; do _path=$(command -v "$_py" 2>/dev/null) || continue; "$_path" -c "import sys; sys.exit(0)" 2>/dev/null && echo "$_path" && break; done)
cd "$PROJECT_ROOT/scripts"
PYTHONIOENCODING=utf-8 "$PYTHON" -m suh_template.suh_command update-pr {owner} {repo} {pr_number} "{릴리스 노트 파일 경로}"
```
...
- 모든 로직은 재사용 스크립트 `scripts/suh_template/suh_command.py`의 `actions` 서브커맨드에 있다. **인라인 Python 작성 금지.**
- **입력 해석은 agent(너)의 책임**이다. 사용자가 주는 URL·PR번호·브랜치명·빈 입력을 보고 아래 라우팅 표에 따라 적절한 서브커맨드와 인자를 결정한다.
- `suh_command`는 **명확한 인자만** 받는다 (URL을 파싱하지 않는다). 출력은 **언제나 JSON**이며 `ok`·데이터·`next`(이어서 호출할 다음 서브커맨드 힌트) 필드를 담는다.
...
PAT는 `suh_command`가 config.json에서 자동 로드하므로 `export GITHUB_PAT`는 생략 가능하다(환경변수가 있으면 우선 사용). `PYTHONIOENCODING=utf-8` 필수(한글 출력 보호).
...
PYTHONIOENCODING=utf-8 "$PYTHON" -m suh_template.suh_command actions show-run {owner} {repo} {run_id}

# 실패 job 로그 (error 라인 필터, Azure redirect 자동 처리)
PYTHONIOENCODING=utf-8 "$PYTHON" -m suh_template.suh_command actions joblog {owner} {repo} {job_id}
#   --grep ".dart"  : 특정 키워드 라인만 (기본 "error")
#   --tail 30       : 매칭 라인 끝 N개 (기본 30)

# 최근 실패 run 목록
PYTHONIOENCODING=utf-8 "$PYTHON" -m suh_template.suh_command actions list-failed {owner} {repo} --limit 10

# PR → 연결된 run 추적
PYTHONIOENCODING=utf-8 "$PYTHON" -m suh_template.suh_command actions resolve-pr {owner} {repo} {pr_number}

# 브랜치 → 최근 run 목록
PYTHONIOENCODING=utf-8 "$PYTHON" -m suh_template.suh_command actions resolve-branch {owner} {repo} "{branch}" --limit 10
```

> **Windows 주의**: `cd "$PROJECT_ROOT/scripts"` 후 `-m suh_template.suh_command`로 실행한다. 임시 파일 파싱·curl 파이프 Python·heredoc 보간은 사용하지 않는다 (Windows Git Bash에서 깨짐). 인자는 모두 명령행/환경변수로 전달한다.
...
출력은 `suh_command explore`의 JSON을 그대로 파싱해 판단한다.
...
PYTHON=$(for _py in python3 python; do _path=$(command -v "$_py" 2>/dev/null) || continue; "$_path" -c "import sys; sys.exit(0)" 2>/dev/null && echo "$_path" && break; done)
cd "$PROJECT_ROOT/scripts"
PYTHONIOENCODING=utf-8 "$PYTHON" -m suh_template.suh_command explore list-repos {owner} --type auto
...
```bash
PYTHONIOENCODING=utf-8 "$PYTHON" -m suh_template.suh_command explore repo-detail {owner} {repo}
```

**2-2. README**

```bash
PYTHONIOENCODING=utf-8 "$PYTHON" -m suh_template.suh_command explore readme {owner} {repo}
```

**2-3. 언어 구성**

```bash
PYTHONIOENCODING=utf-8 "$PYTHON" -m suh_template.suh_command explore languages {owner} {repo}
```

**2-4. 최근 커밋 10개**

```bash
PYTHONIOENCODING=utf-8 "$PYTHON" -m suh_template.suh_command explore commits {owner} {repo} --limit 10
```
...
PYTHON=$(for _py in python3 python; do _path=$(command -v "$_py" 2>/dev/null) || continue; "$_path" -c "import sys; sys.exit(0)" 2>/dev/null && echo "$_path" && break; done)
cd "$PROJECT_ROOT/scripts"
PYTHONIOENCODING=utf-8 "$PYTHON" -m suh_template.suh_command secrets list {owner} {repo}
```
...
`suh_command secrets set`이 PyNaCl로 GitHub public key 암호화 후 PUT한다. PyNaCl 미설치 시 자동 설치를 시도하고, 실패하면 `code:"pynacl_missing"` JSON을 반환한다.

```bash
PYTHON=$(for _py in python3 python; do _path=$(command -v "$_py" 2>/dev/null) || continue; "$_path" -c "import sys; sys.exit(0)" 2>/dev/null && echo "$_path" && break; done)
cd "$PROJECT_ROOT/scripts"
SECRET_VALUE="{secret_value}" PYTHONIOENCODING=utf-8 "$PYTHON" -m suh_template.suh_command secrets set {owner} {repo} {secret_name}
```
