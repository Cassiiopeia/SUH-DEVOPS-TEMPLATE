---
name: suh-report
description: "Report Mode - 구현 보고서 생성 전문가. Git diff와 이슈 분석을 통해 구현 내용을 정리한 보고서를 생성하고 GitHub PR 댓글로 자동 포스팅한다. 기능 흐름은 mermaid 플로우차트로 시각화한다. 구현 완료 후 보고서가 필요할 때, PR 설명 작성 시 사용. /suh-report 호출 시 사용."
---
...
- ⚠️ **[초가독성 수칙] flowchart LR (좌→우) 절대 금지 / TD (위→아래) 100% 강제**: 가로형 흐름도(`flowchart LR`)는 뷰포트가 좁은 모바일 화면 및 pi 하네스 UI 환경에서 가로 스크롤을 무한대로 발생시켜 가독성을 극도로 해친다. 분기 깊이가 아무리 얕고 단순하더라도 **무조건 위에서 아래로 정갈하게 하강하는 `flowchart TD` (Top-to-Bottom) 흐름만을 100% 강제 사용**해야 한다.
