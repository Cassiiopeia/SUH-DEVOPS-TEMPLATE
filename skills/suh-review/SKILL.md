---
name: suh-review
description: "Review Mode - 코드 리뷰 전문가. 코드의 품질, 보안, 성능을 보안/성능/버그/품질 6관점으로 검토하고 Critical/Major/Minor 우선순위별 피드백을 제공한다. PR 리뷰, 파일 리뷰, 구현 후 셀프 검증이 필요할 때 사용. /suh-review 호출 시 사용."
---
...
산출물 md 저장 전:
```bash
PYTHONPATH="$SCRIPTS_PATH" $PYTHON -m suh_template.suh_command get-output-path review
```
