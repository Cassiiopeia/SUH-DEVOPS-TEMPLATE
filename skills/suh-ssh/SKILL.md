...
**이때는 쓰지 않는다**: 시놀로지 DSM 역방향 프록시·도메인 설정 변경 → `cassiiopeia:suh-synology-expose` 사용.
...
Config 파일: `{HOME}/.suh-template/config/config.json` — `ssh` 섹션 사용. **이 고정 경로를 Read tool로 바로 읽는다 — `ls`·glob으로 탐색하거나 플러그인 캐시를 뒤지지 마라.**
