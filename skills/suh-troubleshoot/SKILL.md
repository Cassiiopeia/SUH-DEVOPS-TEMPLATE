---
name: suh-troubleshoot
description: "Troubleshoot Mode - 디버깅 전문가. 가설-검증 방식으로 문제의 근본 원인을 찾고 Quick Fix와 Root Fix를 함께 제시한다. 에러 해결, 버그 디버깅, 문제 진단, 크래시 분석, 성능 문제 해결이 필요할 때 사용. /suh-troubleshoot 호출 시 사용."
---
...
SCRIPTS=$(ls -d ~/.claude/plugins/cache/*/cassiiopeia/*/skills/suh-troubleshoot/scripts 2>/dev/null | sort -V | tail -1); [ -z "$SCRIPTS" ] && SCRIPTS="$PROJECT_ROOT/skills/suh-troubleshoot/scripts"; cd "$SCRIPTS" || exit 1
