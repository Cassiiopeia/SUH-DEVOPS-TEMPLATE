...
import {
  markerForType, existingMarkerInDir, findTypePathCandidates, resolveProjectPaths,
...
test("resolveProjectPaths 대화형: 복수 후보 select 경로", async () => {
  ...
  try {
    ...
    assert.deepEqual(options.map((o) => o.value), ["admin", "web", "직접 입력"]);
  } finally { rmSync(root, { recursive: true, force: true }); }
});
...
test("resolveProjectPaths 대화형: '직접 입력' 선택 → 마커 없는 경로는 경고 후 강제확인", async () => {
  ...
  try {
    touch(root, "web/package.json");
    touch(root, "admin/package.json");
    // select에서 직접 입력 → text로 마커 없는 경로 → confirm(강제) 예
    const io = stubIo({ selects: ["직접 입력"], texts: ["nowhere"], confirms: [true] });
    const map = await resolveProjectPaths({ root, types: ["react"], tty: true, io });
    assert.equal(map.get("react"), "nowhere");
    assert.equal(io.calls.confirm.length, 1); // 강제확인 1회
  } finally { rmSync(root, { recursive: true, force: true }); }
});

test("resolveProjectPaths 대화형: 후보 1개 확인 '아니오' → 직접 입력 루프", async () => {
  const root = makeTmp();
  try {
    touch(root, "app/pubspec.yaml");
    touch(root, "app/lib/main.dart");
    touch(root, "other/pubspec.yaml"); // lib 없어 후보는 아니지만 마커는 실재
    // confirm(후보수락) 아니오 → text "other/" (정규화 검증: 끝 슬래시 제거)
    const io = stubIo({ confirms: [false], texts: ["other/"] });
    const map = await resolveProjectPaths({ root, types: ["flutter"], tty: true, io });
