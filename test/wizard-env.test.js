...
// #494 — 치환 트레이스 수집: 전후값이 파일 최종형과 동일 규칙으로 기록된다
test("substituteEnv collectSubs: key/action/before/after 수집 (#494)", () => {
  const c = '  VOLUME: "/v/__PROJECT_NAME__"  # @wizard ask:/v/__PROJECT_NAME__';
  const subs = [];
  substituteEnv(c, { useDefaults: true, repoName: "my-repo", collectSubs: subs });
  assert.deepEqual(subs, [{ key: "VOLUME", action: "ask", before: "/v/__PROJECT_NAME__", after: "/v/my-repo" }]);
});
